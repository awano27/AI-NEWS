...
def score(item):
    ...
    import re as _re
    engineer = 1.0 if _re.search(KEYWORDS_ENGINEER, t, _re.I) else 0.0
    biz = 1.0 if _re.search(KEYWORDS_BIZ, t, _re.I) else 0.0
    policy = 1.0 if _re.search(KEYWORDS_POLICY, t, _re.I) else 0.0
    big = 1.0 if any(n.lower() in t.lower() for n in BIG_NAMES) else 0.0
    surprise = 1.0 if _re.search(r"(突破|leak|爆|倍|破る|破竹|unprecedented|重大|障害|停止|重大脆弱性|過去最大)", t, _re.I) else 0.0
